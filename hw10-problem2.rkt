...
; max-length : [BinTree-of String] -> Number
...
(define (max-length btos)
  (reduce-tree max string-length btos))
...
; flatten : [BinTree-of String] -> [List-of String]
...
; map-tree : (X Y) [BinTree-of X] [X -> Y] -> [BinTree-of Y]
...
(define (map-tree botx f)
  (reduce-tree make-node (λ (z) (make-leaf (f z))) botx))
