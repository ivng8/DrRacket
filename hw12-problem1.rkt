...
(define (count-strings los)
  (local [; char-counter : String [List-of String] -> SC
          ; Creates a StringCount of a single element
          ; in a list of Strings
          (define (char-counter x lox)
            (cond
              [(empty? lox) 0]
              [(cons? lox)
               (+
                (if (string=? x (first lox)) 1 0)
                (char-counter x (rest lox)))]))]
    (cond
      [(empty? los) '()]
      [(cons? los)
       (cons
        (make-sc
         (first los)
         (char-counter (first los) los))
        (count-strings (filter (λ (x) (not (string=? x (first los)))) los)))])))
=
