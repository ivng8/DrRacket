...
; takes a word and tests if it is a palindrome

(check-expect (palindrome? "mom") #t)
(check-expect (palindrome? "Mom") #t)
(check-expect (palindrome? "dom") #f)
(check-expect (palindrome? "dad") #t)

(define (palindrome? s)
  (string-ci=?
   s
   (foldl string-append "" (explode s))))
...
(check-expect ((ls->color-fn-new LS-WRONG) 50) (square 50 "solid" "dimgray"))
(check-expect ((ls->color-fn-new LS-MISPLACED) 50) (square 50 "solid" "goldenrod"))
(check-expect ((ls->color-fn-new LS-RIGHT) 50) (square 50 "solid" "darkgreen"))

; ls->color-fn-new : LetterStatus -> [NonNegReal -> Image]
; associates a status with a function to produce a background color

(define (ls->color-fn-new ls)
...
; stack/h-new : [List-of Image] -> Image
; space-separates a list of images horizontally
...
(define (stack/h-new images)
  (stack images beside))

; stack/v-new : [List-of Images] -> Image
; space-separates a list of images vertically
...
(define (stack/v-new images)
  (stack images above))

; stack : [List-of Image] -> [Image -> Image
; space-separates a list of images in a certain direction

(define (stack loi x)
  (foldr
   (λ (i old-i) (x GAP i old-i))
   GAP
   loi))
