...
(define (score x y)
  (local [; ls? : [List-of 1String] [List-of 1String] -> [List-of LS]
          ; takes 2 lists of 1Strings and
          ; creates a list of Letter Status
          ; of the first list compared to the second list
          (define (ls? lox loy)
            (cond
              [(string=? lox loy) LS-RIGHT]
              [(and
                (not (string=?  lox loy))
                (string-contains? lox y)) LS-MISPLACED]
              [else LS-WRONG]))]
    (mymap2
     (explode x)
     (mymap2 (explode x) (explode y) ls?)
     make-pair)))
...
(define (partial-guess->full-guess s nat)
  (implode (append
            (explode s)
            (build-list
             (- nat (string-length s))
...
(define (key-wordle ws ke los nat)
  (cond
    [(number? ke) ws]
    [(key=? ke "\b")
     (backspace-helper ws)]
    ...
    [(string-alphabetic? ke)
     (letter-helper ws ke nat)]
    [else ws]))

; backspace-helper : WS -> WS
; takes a wordle state and gives a new or current one
; when backspace is pressed

(check-expect
 (backspace-helper WS-START)
 WS-START)

(check-expect
 (backspace-helper WS-TYPE1)
 WS-START)

(check-expect
 (backspace-helper WS-TYPE2)
 WS-TYPE1)

(check-expect
 (backspace-helper WS-TYPE3)
 WS-TYPE2)

(check-expect
 (backspace-helper WS-TYPE-BAD)
 WS-TYPE2)

(check-expect
 (backspace-helper WS-WORD1)
 WS-WORD1)

(define (backspace-helper ws)
  (make-wordle
   (wordle-past ws)
   (if (= (string-length (wordle-current ws)) 0) (wordle-current ws)
       (substring (wordle-current ws)
                  0
                  (- (string-length (wordle-current ws)) 1)))))

; enter-helper : WS [List-of String] -> WS
; takes a wordle state and gives a new or current one
; when enter is pressed

(check-expect
 (enter-helper WS-START LITTLE-WORDS)
 WS-START)

(check-expect
 (enter-helper WS-TYPE1 LITTLE-WORDS)
 WS-TYPE1)

(check-expect
 (enter-helper WS-TYPE2 LITTLE-WORDS)
 WS-TYPE2)

(check-expect
 (enter-helper WS-TYPE3 LITTLE-WORDS)
 WS-WORD1)

(check-expect
 (enter-helper WS-TYPE-BAD LITTLE-WORDS)
 WS-TYPE-BAD)

(check-expect
 (enter-helper WS-WORD1 LITTLE-WORDS)
 WS-WORD1)

(define (enter-helper ws los)
  (if (string-in-list? (wordle-current ws) los)
      (make-wordle
       (append (wordle-past ws) (list (wordle-current ws)))
       "")
      ws))

; letter-helper : WS KeyEvent Nat -> WS
; takes a wordle state and gives a new or current one
; when a letter is pressed

(check-expect
 (letter-helper WS-START "C" 3)
 WS-TYPE1)
...
(check-expect
 (letter-helper WS-TYPE1 "A" 3)
 WS-TYPE2)

(check-expect
 (letter-helper WS-TYPE1 "A" 1)
 WS-TYPE1)

(check-expect
 (letter-helper WS-TYPE2 "T" 3)
 WS-TYPE3)

(check-expect
 (letter-helper WS-TYPE2 "X" 3)
 WS-TYPE-BAD)

(check-expect
 (letter-helper WS-TYPE3 "S" 3)
 WS-TYPE3)

(define (letter-helper ws ke nat)
  (if (= (string-length (wordle-current ws)) nat) ws
      (make-wordle
       (wordle-past ws)
       (implode (append
                 (explode (wordle-current ws))
                 (list (string-upcase ke)))))))
...
(define (end-wordle? ws s nat)
  (or
   (= (length (wordle-past ws)) nat)
   (string-in-list? s (wordle-past ws))))
...
(check-expect (draw-end-wordle WS-WORD1 "BAD" 1)
              (text "You lost :(" 15 "blue"))
(check-expect (draw-end-wordle WS-WORD1 "CAT" 3)
              (text "You won and it took you 1 try!!!" 15 "blue"))
(check-expect (draw-end-wordle WS-WORD2 "BAD" 2)
              (text "You lost :(" 15 "blue"))
(check-expect (draw-end-wordle WS-WORD2 "DAB" 2)
              (text "You won and it took you 2 tries!!!" 15 "blue"))
(check-expect (draw-end-wordle WS-WORD3 "BAD" 5)
              (text "You won and it took you 3 tries!!!" 15 "blue"))

(define (draw-end-wordle ws s nat)
  (text
   (if
    (not (string-in-list? s (wordle-past ws)))
    "You lost :("
    (string-append "You won and it took you "
                   (number->string (length (wordle-past ws)))
                   " "
                   (if (= (length (wordle-past ws)) 1)
                       "try"
                       "tries")
                   "!!!"))
   15
...
(check-expect (draw-wordle WS-START "CAT" 5)
              (stack/v
               (build-list 5
                           (λ (x) (partial-guess->image "" 3)))))

(check-expect (draw-wordle WS-TYPE1 "DAB" 5)
              (stack/v
               (append
                (list (partial-guess->image "C" 3))
                (build-list 4
                            (λ (x) (partial-guess->image "" 3))))))
...
(check-expect (draw-wordle WS-TYPE2 "CAT" 3)
              (stack/v
               (append
                (list (partial-guess->image "CA" 3))
                (build-list 2
                            (λ (x) (partial-guess->image "" 3))))))

(check-expect (draw-wordle WS-TYPE3 "CAT" 3)
              (stack/v
               (append
                (list (partial-guess->image "CAT" 3))
                (build-list 2
                            (λ (x) (partial-guess->image "" 3))))))

(check-expect (draw-wordle WS-TYPE-BAD "CAT" 4)
              (stack/v
               (append
                (list (partial-guess->image "CAX" 3))
                (build-list 3
                            (λ (x) (partial-guess->image "" 3))))))

(check-expect (draw-wordle WS-WORD1 "DAB" 4)
              (stack/v
               (append
                (list (row->image
                       (map (λ (y) (make-pair y lsp->image)) (score "CAT" "DAB"))))
                (build-list 3
                            (λ (x) (partial-guess->image "" 3))))))

(check-expect (draw-wordle WS-WORD2 "ETA" 3)
              (stack/v
               (append
                (list (row->image
                       (map (λ (y) (make-pair y lsp->image)) (score "CAT" "ETA")))
                      (row->image
                       (map (λ (y) (make-pair y lsp->image)) (score "DAB" "ETA"))))
                (build-list 1
                            (λ (x) (partial-guess->image "" 3))))))

(check-expect (draw-wordle WS-WORD3 "ETA" 5)
              (stack/v
               (append
                (list (row->image
                       (map (λ (y) (make-pair y lsp->image)) (score "CAT" "ETA")))
                      (row->image
                       (map (λ (y) (make-pair y lsp->image)) (score "DAB" "ETA")))
                      (row->image
                       (map (λ (y) (make-pair y lsp->image)) (score "BAD" "ETA"))))
                (build-list 2
                            (λ (x) (partial-guess->image "" 3))))))

(define (draw-wordle ws s nat)
  (stack/v
   (append
    (map row->image
         (map
          (λ (z) (map (λ (y) (make-pair y lsp->image)) z))
          (map (λ (x) (score x s)) (wordle-past ws))))
    (list (partial-guess->image (wordle-current ws) (string-length s)))
    (build-list
     (- nat (+ 1 (length (wordle-past ws))))
     (λ (x) (partial-guess->image "" (string-length s)))))))
