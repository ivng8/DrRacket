...
; border-square: Real -> Image
; Creates a square that will be overlayed with the main image to create an outline
; with an inputted square size and color

(define (border-square x border-color)
  (square (+ x 20) "solid" border-color))

; inside-square: Real -> Image
; Creates the inside square with an inputted size and color

(define (inside-square x box-color)
  (square x "solid" box-color))

; main-square: 1String Real -> Image
; Adds the letter inside of the inside square using the square size as its font
; to create a proportional text with its own inputted color inside of the inside-square
; with an inputted size and color

(define (main-square x letter letter-color box-color)
  (overlay 
   (text letter x letter-color)
   (inside-square x box-color)))

; boxed-letter: Real 1String String String String -> Image
; Creates an image with a letter that has a font and color inside 
; a square with an inputted side-length and inputted color
; With a border of an inputted color

(define (boxed-letter x letter letter-color box-color border-color)
  (overlay
   (main-square x letter letter-color box-color)
   (border-square x border-color)))
